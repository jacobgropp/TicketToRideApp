...
public class ParseDoubleHandler implements HttpHandler{
    ...
    public void handle(HttpExchange httpExchange) throws IOException {
        ...
        try{
            if(httpExchange.getRequestMethod().toLowerCase().equals("post")){
                ...
                DoubleRequest request = (DoubleRequest)convertJsonToObject(requestBody,
                        new DoubleRequest("initial"));

                //Send the request along for processing. Store the response.
                StringProcessor processor = new StringProcessor();
                DoubleResponse response = new DoubleResponse(processor.parseDouble(request.getInitialString()));
